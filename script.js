...
var fieldGameOver, fieldDistance, fieldFinalDist, fieldStart;
...
Hero.prototype.nod = function () {
  var _this = this;
  if (gameStatus != "play" && gameStatus != "waiting") return;
...
Monster.prototype.nod = function () {
  var _this = this;
  if (gameStatus != "play" && gameStatus != "gameOver") return;
...
function gameOver() {
  ...
  fieldFinalDist.innerHTML = Math.floor(distance / 2);
  gameStatus = "gameOver";
  monster.sit();
  hero.hang();
...
function init(event) {
  ...
  createObstacle();
  initUI();
  // resetGame(); // Removed to allow start screen
  loop();
}
...
function initUI() {
  ...
  fieldGameOver = document.getElementById("game-over-overlay");
  fieldFinalDist = document.getElementById("final-dist");
  fieldStart = document.getElementById("start-overlay");
  document.getElementById("restart-btn").onclick = replay;
